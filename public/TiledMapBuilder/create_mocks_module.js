...
MockModule = {
	...
	*/
	...
	getTileset: function(gid) {
		var tilesets = this._source.tilesets;
		for(var i = 0; i < tilesets.length; i++) {
			var tileset = tilesets[i];
			var firstgid = tileset.firstgid;
			var lastgid = firstgid + (tileset.imagewidth / (tileset.tilewidth + tileset.spacing)) *
    ...
    /*
	 * Get the tileproperties of some gid.
	 * @return {Integer} gid AAAARGH
	 */
	getTileTilesetProperties: function(gid) {
		var tileset = this.getTileset(gid);
		var tileproperties = tileset["tileproperties"];
		return gid in tileproperties ? tileproperties[gid] : null;
	},
	...
	createMockEntity:function( layer, dataIndex) {
		...
		var gid = layer.data[dataIndex];

		var mock = {
			type: "Tile",
			head: ["2D", this._renderMethod, "Tile", "Tile" + gid, layer.name].join(','),
			properties: layer.properties,
			tileproperties: this.getTileTilesetProperties(gid),
		};
